...
class CSVGenerator:
    ...
    def get_data_header(self) -> None:
        self.data_header = self.param_names + [self.final_col_name1, self.final_col_name2]

    def get_optimization_type(self) -> None:
        self.optimization_type = st.selectbox('Select optimization type:', ['Single', 'Multi'])
        if self.optimization_type == 'Single':
            ...
            self.final_col_name1 = st.text_input('Name of the first objective:', 'output1')
            self.final_col_name2 = st.text_input('Name of the second objective:', 'output2')
            self.data_header = self.param_names + [self.final_col_name1, self.final_col_name2]

    def get_parameter_ranges(self) -> None:
        for i in range(len(self.param_names)):
            if self.param_types[i] == 'Integer':
                min_val = st.number_input(f'Minimum value for {self.param_names[i]}:', value=0)
                max_val = st.number_input(f'Maximum value for {self.param_names[i]}:', value=100)
                self.param_ranges.append((min_val, max_val))
            elif self.param_types[i] == 'Float':
                min_val = st.number_input(f'Minimum value for {self.param_names[i]}:', value=0.0)
                max_val = st.number_input(f'Maximum value for {self.param_names[i]}:', value=100.0)
                self.param_ranges.append((min_val, max_val))
            else:
                categories = st.text_input(f'Enter categories for {self.param_names[i]} (comma-separated):', 'cat1,cat2,cat3')
